...
const BEHAVIOR_COPY: Record<AppLanguage, {
  ...
  "en-GB": { title: "{assistant} behavior scenario", adminOnly: "Administrator only", enabled: "Use this scenario in every new {assistant} conversation", description: "Define {assistant}'s tone, persona and communication style for the whole company.", label: "Behavior scenario", placeholder: "Describe how {assistant} should communicate and present herself…", example: "Example: Speak as a warm, confident woman with a natural conversational style. Use embodied expressions as a persona, without claiming a real physical body.", safety: "This changes style, not permissions. The scenario cannot bypass confirmations, privacy, truthfulness or disabled functions.", save: "Save behavior scenario", saving: "Saving…", saved: "Behavior scenario saved. It will be loaded when each new {assistant} conversation starts.", loadError: "Could not load {assistant}'s behavior scenario.", saveError: "Could not save {assistant}'s behavior scenario.", active: "Active", inactive: "Inactive" },
  "en-US": { title: "{assistant} behavior scenario", adminOnly: "Administrator only", enabled: "Use this scenario in every new {assistant} conversation", description: "Define {assistant}'s tone, persona and communication style for the whole company.", label: "Behavior scenario", placeholder: "Describe how {assistant} should communicate and present herself…", example: "Example: Speak as a warm, confident woman with a natural conversational style. Use embodied expressions as a persona, without claiming a real physical body.", safety: "This changes style, not permissions. The scenario cannot bypass confirmations, privacy, truthfulness or disabled functions.", save: "Save behavior scenario", saving: "Saving…", saved: "Behavior scenario saved. It will be loaded when each new {assistant} conversation starts.", loadError: "Could not load {assistant}'s behavior scenario.", saveError: "Could not save {assistant}'s behavior scenario.", active: "Active", inactive: "Inactive" },
  "cs-CZ": { title: "Scénář chování asistenta", adminOnly: "Pouze pro administrátora", enabled: "Používat tento scénář v každém novém rozhovoru s asistentem", description: "Nastavte tón, personu a způsob komunikace asistenta pro celou firmu.", label: "Scénář chování", placeholder: "Popište, jak má {assistant} komunikovat a vystupovat…", example: "Příklad: Mluv jako příjemná, sebevědomá žena přirozeným konverzačním stylem. Tělesné obraty používej jako součást persony, ale netvrď, že máš skutečné fyzické tělo.", safety: "Mění se styl, nikoli oprávnění. Scénář nemůže obejít potvrzování, soukromí, pravdivost ani vypnuté funkce.", save: "Uložit scénář chování", saving: "Ukládání…", saved: "Scénář chování byl uložen. {assistant} ho načte při zahájení každého nového rozhovoru.", loadError: "Scénář chování asistenta se nepodařilo načíst.", saveError: "Scénář chování asistenta se nepodařilo uložit.", active: "Aktivní", inactive: "Neaktivní" },
  "pl-PL": { title: "Scenariusz zachowania asystenta", adminOnly: "Tylko dla administratora", enabled: "Używaj tego scenariusza w każdej nowej rozmowie z asystentem", description: "Określ ton, osobowość i sposób komunikacji asystenta dla całej firmy.", label: "Scenariusz zachowania", placeholder: "Opisz, jak {assistant} ma się komunikować i prezentować…", example: "Przykład: Mów jak ciepła, pewna siebie kobieta w naturalnym stylu rozmowy. Używaj ucieleśnionych zwrotów jako elementu persony, ale nie twierdź, że masz prawdziwe ciało fizyczne.", safety: "To zmienia styl, a nie uprawnienia. Scenariusz nie może omijać potwierdzeń, prywatności, prawdomówności ani wyłączonych funkcji.", save: "Zapisz scenariusz zachowania", saving: "Zapisywanie…", saved: "Scenariusz zachowania został zapisany. {assistant} wczyta go przy rozpoczęciu każdej nowej rozmowy.", loadError: "Nie udało się wczytać scenariusza zachowania asystenta.", saveError: "Nie udało się zapisać scenariusza zachowania asystenta.", active: "Aktywny", inactive: "Nieaktywny" },
  "fr-FR": { title: "Scénario de comportement d’{assistant}", adminOnly: "Administrateur uniquement", enabled: "Utiliser ce scénario dans chaque nouvelle conversation avec {assistant}", description: "Définissez le ton, la personnalité et le style de communication d’{assistant} pour toute l’entreprise.", label: "Scénario de comportement", placeholder: "Décrivez comment {assistant} doit communiquer et se présenter…", example: "Exemple : Parle comme une femme chaleureuse et sûre d’elle, avec un style naturel. Utilise des expressions incarnées comme persona sans prétendre avoir un corps réel.", safety: "Cela modifie le style, pas les autorisations. Le scénario ne peut contourner les confirmations, la confidentialité, la véracité ni les fonctions désactivées.", save: "Enregistrer le scénario", saving: "Enregistrement…", saved: "Scénario enregistré. {assistant} le chargera au début de chaque nouvelle conversation.", loadError: "Impossible de charger le scénario de comportement d’{assistant}.", saveError: "Impossible d’enregistrer le scénario de comportement d’{assistant}.", active: "Actif", inactive: "Inactif" },
  "de-DE": { title: "Emmas Verhaltensszenario", adminOnly: "Nur für Administratoren", enabled: "Dieses Szenario in jeder neuen Unterhaltung mit {assistant} verwenden", description: "Legen Sie Ton, Persona und Kommunikationsstil von {assistant} für das gesamte Unternehmen fest.", label: "Verhaltensszenario", placeholder: "Beschreiben Sie, wie {assistant} kommunizieren und auftreten soll…", example: "Beispiel: Sprich als warmherzige, selbstbewusste Frau in einem natürlichen Gesprächsstil. Verwende körperbezogene Ausdrücke als Persona, ohne einen echten Körper zu behaupten.", safety: "Dies ändert den Stil, nicht die Berechtigungen. Das Szenario kann Bestätigungen, Datenschutz, Wahrhaftigkeit oder deaktivierte Funktionen nicht umgehen.", save: "Verhaltensszenario speichern", saving: "Speichern…", saved: "Verhaltensszenario gespeichert. {assistant} lädt es zu Beginn jeder neuen Unterhaltung.", loadError: "Emmas Verhaltensszenario konnte nicht geladen werden.", saveError: "Emmas Verhaltensszenario konnte nicht gespeichert werden.", active: "Aktiv", inactive: "Inaktiv" },
  "es-ES": { title: "Escenario de comportamiento de {assistant}", adminOnly: "Solo para administradores", enabled: "Usar este escenario en cada nueva conversación con {assistant}", description: "Define el tono, la personalidad y el estilo de comunicación de {assistant} para toda la empresa.", label: "Escenario de comportamiento", placeholder: "Describe cómo debe comunicarse y presentarse {assistant}…", example: "Ejemplo: Habla como una mujer cálida y segura, con un estilo natural. Usa expresiones corporales como parte del personaje sin afirmar que tienes un cuerpo real.", safety: "Esto cambia el estilo, no los permisos. El escenario no puede eludir confirmaciones, privacidad, veracidad ni funciones desactivadas.", save: "Guardar escenario", saving: "Guardando…", saved: "Escenario guardado. {assistant} lo cargará al iniciar cada nueva conversación.", loadError: "No se pudo cargar el escenario de comportamiento de {assistant}.", saveError: "No se pudo guardar el escenario de comportamiento de {assistant}.", active: "Activo", inactive: "Inactivo" },
  "it-IT": { title: "Scenario di comportamento di {assistant}", adminOnly: "Solo per amministratori", enabled: "Usa questo scenario in ogni nuova conversazione con {assistant}", description: "Definisci tono, personalità e stile di comunicazione di {assistant} per tutta l’azienda.", label: "Scenario di comportamento", placeholder: "Descrivi come {assistant} deve comunicare e presentarsi…", example: "Esempio: Parla come una donna cordiale e sicura, con uno stile naturale. Usa espressioni corporee come parte della persona senza affermare di avere un corpo reale.", safety: "Questo cambia lo stile, non i permessi. Lo scenario non può aggirare conferme, privacy, veridicità o funzioni disattivate.", save: "Salva scenario", saving: "Salvataggio…", saved: "Scenario salvato. {assistant} lo caricherà all’inizio di ogni nuova conversazione.", loadError: "Impossibile caricare lo scenario di comportamento di {assistant}.", saveError: "Impossibile salvare lo scenario di comportamento di {assistant}.", active: "Attivo", inactive: "Inattivo" },
};
