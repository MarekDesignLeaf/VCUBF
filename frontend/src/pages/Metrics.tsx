...
export function Metrics() {
  ...
  return <div>
    ...
    <p className="hint">{data.invoicing.basis}</p>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))", gap: 16 }}>
      <section className="card"><h3>Issued invoices</h3><strong style={{ fontSize: 28 }}>£{data.invoicing.issued.valueGbp.current.toFixed(2)}</strong><p>{`${data.invoicing.issued.count.current} invoices issued · ${change(data.invoicing.issued.valueGbp.current, data.invoicing.issued.valueGbp.previous, " GBP")}`}</p></section>
      <section className="card"><h3>Payments received</h3><strong style={{ fontSize: 28 }}>£{data.invoicing.paymentsReceived.valueGbp.current.toFixed(2)}</strong><p>{`${data.invoicing.paymentsReceived.count.current} payments recorded · ${change(data.invoicing.paymentsReceived.valueGbp.current, data.invoicing.paymentsReceived.valueGbp.previous, " GBP")}`}</p></section>
      <section className="card"><h3>Outstanding at period end</h3><strong style={{ fontSize: 28 }}>£{data.invoicing.outstanding.balanceGbp.toFixed(2)}</strong><p>{`${data.invoicing.outstanding.count} open invoices; ${data.invoicing.outstanding.overdueCount} overdue (£${data.invoicing.outstanding.overdueBalanceGbp.toFixed(2)})`}{data.invoicing.outstanding.withoutDueDateCount ? ` · ${data.invoicing.outstanding.withoutDueDateCount} without a due date` : ""}</p></section>
      <section className="card"><h3>Average days to settle</h3><strong style={{ fontSize: 28 }}>{value(data.invoicing.averageDaysToSettle, " days")}</strong><p>{data.invoicing.settledInvoiceCount ? `${data.invoicing.settledInvoiceCount} invoices settled in this period` : "No invoice was settled in this period"}</p></section>
    </div>
