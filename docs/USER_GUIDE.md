...
Windows Alfonzo is the only voice interface. She listens through the Windows companion, not through the browser, so there is no second browser microphone, wake-word button or **Run** step for normal voice interaction. The default wake word is **Alfonzo**, but you can replace it from **Account** at any time.

**Current Windows runtime — Voice v2.** Install it from `windows-companion` with `Install-VoiceV2.ps1`. It removes the legacy companion runtime and creates one **VCUBF Secretary** desktop icon that opens a dedicated Secretary browser window and starts voice for that window only. Voice v2 detects the wake word fully locally with Picovoice Porcupine (automatic Deepgram wake verification fallback), transcribes speech locally on the Snapdragon Hexagon NPU with Qualcomm Whisper (automatic Deepgram Nova-3 fallback), speaks through ElevenLabs streaming, keeps the microphone open during playback behind acoustic echo cancellation, and executes every request through the same authenticated `/command/assistant` endpoint, permissions, confirmations and audit trail. Audio is never written to disk. Provider keys, the wake-word model and diagnostics are documented in `docs/VOICE_V2_SETUP.md`. The legacy `Install.ps1` companion described further below remains only as a fallback on machines where Voice v2 is not installed.

The collapsed **Type to Alfonzo (optional)** field above each signed-in page is a keyboard fallback only. It sends a defined deterministic text command through the same authenticated backend used by Alfonzo. Unsupported wording is rejected; Secretary does not guess an action. Backend permissions, ambiguity rejection and action-specific confirmation remain active for both typed and spoken requests.

On Windows 11, install the native companion from `windows-companion` with `Install.ps1 -StartNow`. Its tray icon keeps local wake-word recognition active even when the browser is closed. Alfonzo opens the normal VCUBF login in your default browser, where saved-password autofill works. Sign in and approve the matching eight-character code on Account; the pairing expires after ten minutes and can be consumed only once. No password is entered into or saved by the companion, and its API token is encrypted by Windows for the signed-in Windows account. The wake word has a dedicated high-priority local recognition grammar. Say `Alfonzo` alone to enter Realtime listening before speaking naturally, or include an English command in the same phrase. For a same-phrase command, Windows detects activation and the recognised utterance is held in memory only while the authenticated backend obtains a more accurate OpenAI transcription; the WAV is never written to disk or stored by VCUBF. The accurate text is shown in **What Alfonzo hears**, executed, and retained in the text conversation history. The default Windows hands-free mode speaks the result and accepts follow-up speech for twelve seconds; say `stop` to end. The monitor also displays microphone level, local hypotheses and activated Realtime turns; it can be closed and reopened from the tray with **Show live hearing**. Its pre-wake preview stays on the PC and is neither uploaded nor saved in conversation history. Backend permissions, ambiguity rejection and any action-specific confirmation remain active. Allow desktop microphone access in Windows Privacy settings.

Natural assistant mode lets Alfonzo understand ordinary phrasing, hold a short follow-up conversation, ask for missing details and describe a safe plan for a complex business objective. With Realtime disabled, OpenAI receives recognized text and a short six-message conversation window rather than microphone audio. Exact commands remain deterministic. The language model can only propose one of the existing canonical commands; the backend reparses it and remains the only component allowed to execute it. Say, for example, **“Alfonzo, switch language to Czech”** or **“Alfonzo, změň jazyk na češtinu”** to change both Alfonzo and the Secretary navigation menu. Supported languages are English (UK/US), Czech, Polish, French, German, Spanish and Italian. Legal, financial, deletion, publishing, hiring, payment and other risky requests are not converted into direct voice actions and must use their reviewed confirmation flow. Right-click the tray icon for **Talk to Alfonzo now**, listening controls and settings for wake word, language, assistant mode, speech speed and volume.

Alfonzo can send a Gmail email only when exactly one enabled Gmail source has **Send email** permission. Say, for example, `Alfonzo, send email to customer@example.com; subject Quote ready; body Your quote is attached.` Alfonzo repeats the recipient, subject and message preview, then waits for **yes**, **confirm**, **no** or **cancel**. A confirmation sends the reviewed message once; cancellation, transcript deletion, failure or a five-minute expiry removes the temporary pending message content without sending it.

When **Realtime audio and interruption** is enabled, the local recognizer still detects `Alfonzo` before cloud audio starts. The activated conversation then uses a temporary OpenAI speech-to-speech session with automatic voice-activity detection. You do not need to press a button or wait for a beep between follow-ups, and speaking over Alfonzo stops the current response so the new request can be heard. Company requests are executed only by calling back into the authenticated VCUBF command endpoint. The backend OpenAI key is never sent to the PC; the companion receives only a short-lived session credential. Realtime closes after inactivity or three minutes and falls back to the authenticated text assistant if unavailable.

The **Windows Alfonzo** control centre above each signed-in page shows whether the companion is offline, listening, hearing, thinking, speaking or paused. It displays the latest command and answer plus **Saved conversation transcripts**, grouped by activated conversation with every completed user/Emma text turn, order, time, mode and completion state. These text transcripts remain in Secretary until the user chooses **Delete transcript history** and confirms deletion; deletion also ends any active conversation and cancels any pending Alfonzo email so no later turn can send it. Background speech that does not activate the wake word is not uploaded, and VCUBF never stores microphone audio. Choose **Pause listening** whenever Alfonzo must ignore the wake word, **Resume listening** to reactivate it, or **End conversation** to leave Realtime mode while keeping local wake-word listening available.

Alfonzo also receives a backend-maintained complete menu tree: every sidebar item, its detail-screen subtree, purpose, primary controls, prerequisites and safety boundaries. Ask, for example, “Alfonzo, read the full menu”, “Alfonzo, přečti celé menu”, “Alfonzo, read full menu customers and work” or “Alfonzo, where do I create an invoice?”. The **Windows Alfonzo** control centre exposes the same tree under **Complete Secretary menu Alfonzo can read**. She should name the correct page and existing controls, give short ordered steps, identify missing records or permissions, and explain when a preview or confirmation is required. She must not invent pages or claim that a draft, PDF, approval or internal selection was externally sent or published. An automated test fails when a new frontend route or sidebar item is not represented in this map.

The installer also creates a single **VCUBF Secretary** desktop icon. Double-clicking it starts Alfonzo before opening one production sign-in flow in the default browser. The one-time device handoff completes automatically after a successful login, so the browser and companion use the same VCUBF account without a second approval click. On a fresh companion start Alfonzo says that she is active and listening. After the first successful login, the page pre-fills the last-used email on this PC; the browser remains responsible for offering any saved password. The encrypted device session lasts up to 30 days but is revoked by password changes, account disablement or session invalidation.
...
Use **Data Quality** to review duplicate-client and missing-contact evidence. Client merge is confirmation-gated and archives the duplicate. Every confirmed merge is listed under **Merge history**, where **Preview un-merge** shows exactly which records would move back to the duplicate and which can no longer be restored, and **Confirm un-merge** reverses that one merge. Records created on the kept client after the merge, or moved elsewhere since, are never pulled back.
...
**Notifications** aggregates real Secretary signals such as unresolved enquiries, overdue follow-ups/tasks, overload, expiring quotes, overdue invoices, unready job resources, stale leads and possible duplicates. Acknowledging a notification marks it handled; it does not alter the source record. **Daily digest** on the same page is an opt-in per person: it emails a plain-text copy of your own feed to your own account email through the company's authorised Gmail source — never to a client. **Preview digest** shows the exact message and sends nothing; **Confirm and send** sends it once. With the opt-in on, the server also sends it once a day at your chosen hour. An administrator can change the day thresholds behind the quote-expiry, stale-lead, stuck-job and resource-readiness signals on the **Company** page; defaults and allowed ranges are shown there and every change is audited.
