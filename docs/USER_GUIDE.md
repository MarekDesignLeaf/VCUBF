...
Emma also receives a backend-maintained complete menu tree: every sidebar item, its detail-screen subtree, purpose, primary controls, prerequisites and safety boundaries. Ask, for example, “Emma, read the full menu”, “Emma, přečti celé menu”, “Emma, read full menu customers and work” or “Emma, where do I create an invoice?”. The **Windows Emma** control centre exposes the same tree under **Complete Secretary menu Emma can read**. She should name the correct page and existing controls, give short ordered steps, identify missing records or permissions, and explain when a preview or confirmation is required. She must not invent pages or claim that a draft, PDF, approval or internal selection was externally sent or published. An automated test fails when a new frontend route or sidebar item is not represented in this map.
