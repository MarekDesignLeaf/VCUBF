...
export const CONNECTOR_DEFINITIONS: Record<ConnectorKey, ConnectorDefinition> = {
  gmail: {
    ...
    canWrite: ["draft messages", "send messages after explicit confirmation", "move reviewed messages to Trash after explicit confirmation"],
    logicalScopes: ["read:messages", "write:drafts", "send:messages", "delete:messages"],
    requiredPermissions: ["connectors.read", "connectors.manage"],
    returnedDataTypes: ["communication intake", "thread reference"],
    supportedActions: ["authorize selected Gmail access", "synchronise messages into communication intake", "create a draft", "send a reviewed and confirmed message", "move a reviewed source message to Gmail Trash and remove its local intake copy"],
  ...
  whatsapp_business: {
    ...
    returnedDataTypes: ["communication intake", "external sender contact", "CRM contact link", "message delivery reference"],
    supportedActions: ["verify signed Meta webhooks", "import inbound messages", "synchronise inbound sender contacts without overwriting CRM data", "send a reviewed and confirmed text message"],
