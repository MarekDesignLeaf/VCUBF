...
// Maintained product map used by {assistant} for in-app guidance. This contains only
...
- Sign in (/login): enter email and password, then choose Sign in. Saved credentials are offered by the browser or Windows credential provider, not read by {assistant}.
...
- Connectors: **Register data source**, **Authorize Gmail/Contacts/Calendar/Drive/Google Photos**, **Enable**, **Initial sync** or **Sync changes**, **Select Drive images**, **Select Google Photos**, **Review contacts/events/images**, explicitly **Import/Register** selected records, **Write email**, **Create draft only**, **Review and send email**, **Write WhatsApp**, **Review and send WhatsApp**, or **Disconnect**. Guided setup registers missing sources, opens each available provider flow in sequence, resumes after OAuth, asks for the mandatory Enable confirmation and performs the first supported sync. For {assistant} email, require one enabled Gmail source with send:messages, collect recipient/subject/body, read the exact preview and only then accept a separate confirmation. Every email/WhatsApp send still shows the final destination and message and requires a separate confirmation.
...
- {assistant} Memory: enter the exact note, choose For me or For the company, then Remember. Archive removes an active note from {assistant}'s future context without deleting its record. Voice equivalents are “remember that …”, “remember for the company that …” and “what do you remember about …”.
...
- Account: Change password; Voice control lets the user change wake word, {assistant} and Secretary menu language, and continuous-listening preference with Save voice preferences.
