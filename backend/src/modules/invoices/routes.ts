...
import { exportInvoicePdf } from "../../services/invoicePdfService.js";
import { sendInvoiceByEmail } from "../../services/documentDeliveryService.js";
...
invoicesRouter.get("/:id/pdf",requirePermission("crm.read"),async(req,res)=>{const b=await exportInvoicePdf(req.user!,req.params.id);if(!b)return res.status(404).json({error:"INVOICE_NOT_FOUND"});res.set({"Content-Type":"application/pdf","Content-Disposition":`attachment; filename="invoice-${req.params.id}.pdf"`,"Cache-Control":"private, no-store","X-Content-Type-Options":"nosniff"});res.send(b)});
...
invoicesRouter.put("/:id",requirePermission("crm.manage"),async(req,res)=>{const x=await s.updateInvoice(req.user!,req.params.id,req.body);if(!x.ok)return res.status(x.httpStatus).json({error:x.error,message:x.message});res.json(x.data)});
...
// send_invoice_pdf — confirmation-gated (risk 3); only an issued invoice can
// be sent. The preview shows the exact recipients, subject, body and PDF
// attachment; the invoice status never changes and the delivery is recorded
// as an outbound communication.
invoicesRouter.post("/:id/send-email", requirePermission("crm.manage"), async (req, res) => {
  const result = await sendInvoiceByEmail(req.user!, req.params.id, req.body);
